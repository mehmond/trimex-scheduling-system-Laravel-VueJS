    ...
    <div>
        <div class="container pt-3">
            ...
            <Modal
                ...
                :closable="false"
            >
                <Input v-model="data.name" placeholder="Add campus name" />
            ...
            <Modal
                ...
                :closable="false"
            >
                <Input v-model="editData.name" placeholder="Edit campus name" />
...
export default {
    ...
    methods: {
        async store() {
            if (this.data.name.trim() == "")
                return this.error("Campus name is required");
            this.isAdding = true;
            ...
            if (res.status === 201) {
                ...
                this.success("Campus has been added successfully!");
                this.addModal = false;
                this.data.name = "";
            } else {
                if (res.status === 422) {
        ...
        async update() {
            ...
            if (res.status === 200) {
                ...
                this.success("Campus has been edited successfully!");
        ...
        showDeletingModal(item, i) {
            ...
            const deleteModalObj = {
                ...
                deleteUrl: "app/destroy_campus",
                data: { id: item.id },
                deletingIndex: i,
                isDeleted: false,
                msg: "Are you sure you want to delete this campus?",
                successMsg: "Campus has been deleted successfully!"
            };
    ...
    async created() {
        const res = await this.callApi("get", "app/index_campus");
        if (res.status === 200) {
            this.items = res.data;
        } else {
            if (res.status === 404) {
